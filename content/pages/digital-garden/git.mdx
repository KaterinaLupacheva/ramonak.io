...
<CodeBlock>{`git tag -d <tag_name>`}</CodeBlock>

- create tag

<CodeBlock>{`git tag <tag_name>`}</CodeBlock>

- push tags

<CodeBlock>{`git push origin --tags`}</CodeBlock>
...
<CodeBlock>{`git log --graph --oneline --decorate`}</CodeBlock>
